...
for imageNum = 1:numImages
  ...
  for filterNum = 1:numFilters
    ...
    filter = zeros(filterDim,filterDim); % You should replace this
    % Form W, obtain the feature (filterDim x filterDim) needed during the
    % convolution
    %%% YOUR CODE HERE %%%
    filter=filter+W(:,:,filterNum);

    % Flip the feature matrix because of the definition of convolution, as
    % explained earlier
    ...
    %%% YOUR CODE HERE %%%
    resp=conv2(resp.^2,poolMat,'valid');
    resp=resp(1:poolDim:end,1:poolDim:end);
    act=act+sqrt(resp+params.epsilon);
